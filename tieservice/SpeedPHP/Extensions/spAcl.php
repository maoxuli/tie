...
class spAcl
{
	 ...
	 * 
	 * @param aclrole    用户标识：可以是组名或用户名
	 */
	public function set($aclrole)
	{
		$_SESSION["SpAclSession"] = $aclrole;
	}
}
...
class spAclModel extends spModel
{
	 ...
	 * 
	 * @param aclrole    用户标识：可以是组名或是用户名
	 * @param controller    控制器名称
	 * @param action    动作名称
	 */
	public function check($aclrole = SPANONYMOUS, $controller, $action)
	{
		$rows = array('controller' => $controller, 'action' => $action );
		if( $acl = $this->findAll($rows) ){
			foreach($acl as $v){
				if($v["aclrole"] == SPANONYMOUS || $v["aclrole"] == $aclrole)return 1;
			}
			return 0;
		}else{
			return -1;
